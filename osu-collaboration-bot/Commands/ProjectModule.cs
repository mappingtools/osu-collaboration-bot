    ...
    public class ProjectModule : InteractionModuleBase<SocketInteractionContext> {
        ...
        public async Task Create([Summary("project", "The name of the new project")]string projectName) {
            try {
                ...
                _ = await CommonService.GetPersonAsync(_context, _client.Rest, Context.User.Id);
        ...
        public static async Task AddMemberInternal<T>(DiscordSocketRestClient client, SocketInteractionContext context, OsuCollabContext dbContext, Project project, T user) where T : IPresence, IUser {
            ...
            // Make sure a person row exists
            _ = await CommonService.GetPersonAsync(dbContext, client, user.Id);
        ...
        public async Task Tags([RequireProjectManager][Autocomplete(typeof(ProjectAutocompleteHandler))][Summary("project", "The project")]string projectName) {
            ...
            try {
                ...
                string[] namesOrAliases = (await _context.Members.AsQueryable()
                    .Where(o => o.ProjectId == project.Id).Include(o => o.Person).ToListAsync()).Select(o => _resourceService.MemberAliasOrName(o)).ToArray();
